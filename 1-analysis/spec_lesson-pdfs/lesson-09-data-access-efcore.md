...
build; the date is cited in the "Before you start" callout (EF Core releases and planning). Re-capture `CONSOLE_OUT` /
`VB_OUT` whenever a console sample changes (see § 8).

## 3 · Structure

22 pages at the 2026-09-20 fix build (rendered by the workflow's fixer after the two reviews of lesson 09).
...
| 1 | Why data access is where a .NET service shows its age | story (4 ¶) · `kpi` 1.1 (5 tiles: EF Core 10 LTS to Nov 2028 · N+1 21 → 1 · expire 11 → 1 · 23 test cases · InMemory provider discouraged) · `legend` (C#, architecture, legacy) · info callout "Before you start" (4 items: prerequisites ref(4)/ref(8) · the five projects · package and tool versions + `dotnet tool restore` + the EF Core 10 support date with its link · honesty chips and illustrative rates) |
| 2 | The model — DbContext, entities and value objects | story (3 ¶; ¶1 ends with the `IDbContextFactory<T>` / `AddDbContextPool` answer to the concurrency trap) · mapping 2.1 (14 rows: JPA/Hibernate/TypeORM/Flyway → EF Core, including `@Transactional` and TypeORM `migration:generate`) · compare 2.2 (Kotlin JPA entity ↔ C# `QuoteConfiguration`) · code 2.3 (`MotorQuoteDbContext`) · code 2.4 (`Money` record struct + `Driver` record) · mermaid 2.5 erDiagram `direction LR` (schema of the committed migration, all keys plus the columns later sections filter on, caption says "selected columns shown") · cards 2.6 (six mapping tools, 3 + 3 in a "(continued)" band; the data-annotations card says the five attributes are BCL types and that EF's own `[Index]` / `[Owned]` / `[Precision]` live in `Microsoft.EntityFrameworkCore.Abstractions`) |
...
1 twocol · 4 tables besides the mapping (3.7, 5.2, 6.3, 7.6) · 27 distinct source links (every `link()` defined in the
module is used at least once). Pagination at this build: the emptiest pages are 7 and 16 (about 30 % each); every
other page is more than 74 % full, the last page excepted.
...
- **Captured output.** `CONSOLE_OUT` (full `L09.QueryConsole` run) and `VB_OUT` (full `L09.VbReport` run) are pasted
  ...
  they were checked line-for-line against fresh runs on 2026-09-16 and again on 2026-09-20 after the region edits. The
  terminal panels 5.3 and 8.3 were captured from `dotnet ef` 10.0.12 in `L09.Data` and `L09.VbModel` (build-progress
  lines and long URLs left out, long lines wrapped — stated in the panel header).
- **Charts and notes parsed from the captured output (MEASURED):** 3.4 and KPI "Load 20 quotes" = console block 2
  ...
  shown in the 3.3 panel); 6.4, KPI "Expire 10 quotes" and the 6.2 note = block 5 (11 vs 1 commands, 10 rows); the 7.4
  note = block 7. The same numbers are asserted by `QueryShapeTests` and `SavingTests`, so a change in EF Core
  behaviour fails the tests before the PDF goes stale.
- **Computed at build (MEASURED):** KPI "Test cases" and chart 7.5 = `[Fact]` + `[InlineData]` rows per test class
  (23 at this build: QueryShapeTests 10, SavingTests 8, ProviderTrapTests 3, ModelTests 2); chart 9.3 = `loc()` per
  project with `Migrations/` split out (404 hand-written data library, 505 generated migration, 276 tests,
...
- **Bundle size** 33.0 MB (34,632,598 bytes, `BUNDLE_BYTES`) = the framework-dependent `efbundle.exe` produced by
  `dotnet ef migrations bundle` on the build machine (MEASURED, captured).
- **VERIFIED facts (each linked in the PDF):** EF Core 10 supported to 10 Nov 2028, releases aligned with .NET support,
  requires .NET 10 (EF Core releases and planning, What's new in EF Core 10); complex types since EF Core 8,
  ...
  (Creating and configuring a model); DbContext not thread-safe, scoped by `AddDbContext`, `AddDbContextFactory` and
  `AddDbContextPool` (DbContext lifetime); lazy loading is off unless the Proxies package or an injected `ILazyLoader`
...
- **Not run (described from the linked documentation):** SQL Server `rowversion`, PostgreSQL `xmin`, a retrying
  execution strategy with a user transaction, SQL Server statement batching, PostgreSQL and Testcontainers behaviour.
...
- `roster.py`; samples (all project names start `L09.`):
  ...
  `CoverageReport`, `SqlCommandLog` interceptor, `SqliteDb` fixture, deterministic `QuoteSeed` with illustrative
  rates); `L09.Data.Tests` (xUnit, 23 cases, SQLite in-memory, plus the InMemory provider only for the trap test);
...
- Sources: Microsoft Learn — EF Core releases and planning (https://learn.microsoft.com/en-us/ef/core/what-is-new/),
  ...
  lifetime (…/ef/core/dbcontext-configuration/), Lazy loading (…/querying/related-data/lazy), Single vs. split
  ...
  (https://www.nuget.org/packages/<id>); GitHub — Dapper (https://github.com/DapperLib/Dapper); Testcontainers for .NET
...
- The command counts shown (N+1 = n + 1, Include = 1, AsSplitQuery = 2, tracked expire = 11, ExecuteUpdate = 1) must
  stay asserted in `QueryShapeTests` / `SavingTests`.
...
- ⚠ The bundle size and the EF Core patch number (10.0.12) reflect the build machine on 2026-09-16.
...
- ⚠ Pages 7 and 16 are about 30 % empty because the next block (a chart row, a code panel) cannot split; that is inside
  the standard's "about a third" but any content change can move it, so re-check `page-*.png` after edits.
