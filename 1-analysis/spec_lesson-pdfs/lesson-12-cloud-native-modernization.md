...
- Captured figures: re-capture panels 4.5, 7.3 and 8.2 and the `PACKAGES` / `LAYERS` constants when the samples, the
  SDK or the base image change (7.3 was re-captured on 2026-09-20 after the VB6 desktop rule changed). `blocks()` raises if the test-case count drifts from `CAPTURED_TESTS = 16` or the
  estate size drifts from the captured triage run.

## 3 · Structure

22 pages at the 2026-09-20 build (samples and captured figures date from 2026-09-16; the fixer's edits and the runtime table re-check are 2026-09-20).
...
| 1 | Why this lesson — MotorQuote meets the AWS estate you already run | story (4 ¶) · `kpi` 1.1 (5 tiles) · `legend` (Python, cloud, architecture, legacy — only the tags the lesson's cards wear) · info callout "Before you start" (4 items) |
...
| 3 | .NET on Lambda — handlers, Annotations and SnapStart | story (3 ¶) · compare 3.1 (Python Powertools `APIGatewayHttpResolver` ↔ C# Annotations `CreateQuote`; its note says Annotations and Powertools for AWS Lambda (.NET) are complementary) · code 3.2 (`Startup`) · mermaid 3.3 sequenceDiagram (cold INIT then a warm request) · compare 3.4 (raw C# handler ↔ raw VB handler) · code 3.5 (SnapStart hooks: the before-snapshot hook drives ten requests through `RawQuoteHandler`) · table 3.6 (.NET runtimes on Lambda, 5 rows, with a note that `dotnet9` is image-only) · chartrow 3.7 bar (handler code lines) + 3.8 bar (deployment zip size) · code 3.9 (test of the generated wrapper) |
...
| 6 | Observability — OpenTelemetry on .NET's own APIs | story (3 ¶) · code 6.1 (telemetry wiring) · code 6.2 (span + counter + message-template log) · mermaid 6.3 flowchart LR (where each signal goes) · code 6.4 (in-memory exporter span test) |
...
| 8 | Hands-on — and what to learn after the track | story (3 ¶, the third is the after-the-track guidance) · code 8.1 commands · code 8.2 (captured verify_samples + dotnet test summary) · chart 8.3 hbar (code lines per project) · warn "⚠ Traps" (5) · ok "✔ Checkpoint — you have finished the track when you can" (5) · summary "🔍 Check yourself" (7) · footer ("After the track:") |

Standard minimums as built: 8 numbered stories · 1 mapping (15 rows) · 2 card bands (3 cards each) · 6 mermaid
...
- **MEASURED at build time** (recomputed on every render):
  - Chart 3.7: `region_loc()` over regions `annotations`, `raw-cs`, `raw-vb` (17 / 25 / 27 at this build); the note's
    percentage is computed from them.
  - Chart 8.3: `loc()` per sample project over `.cs`/`.vb` files, bin/obj excluded (EstateTriage 63, QuoteContainer 74,
    QuoteContainer.Tests 55, QuoteLambda 133, QuoteLambda.Tests 86, QuoteLambdaVb 42, RatingVb 40); the note's sums are
    computed.
  - Test-case count (KPI-adjacent text, 3.9, §8): `[Fact]` + `[InlineData(` lines in the two test files (11 + 5), checked
...
- **MEASURED — captured from real runs on the build machine (Windows x64, SDK 10.0.401, runtime 10.0.12, 2026-09-16):**
  - Chart 3.8 and KPI "Lambda zip": `dotnet publish -c Release -o <dir>` of `L12.QuoteLambda` (40 files, 807,838 bytes
    zipped with deflate), the same with `-r linux-arm64 --self-contained false` (38 files, 722,956 bytes) and
    `L12.QuoteLambdaVb` (9 files, 77,209 bytes). The zipping was done with Python `zipfile` (ZIP_DEFLATED).
  ...
  - Panel 7.3: `dotnet run` of `L12.EstateTriage`. Panel 8.2: `verify_samples.py --only 12` and the two `dotnet test`
    summary lines (`$SAMPLES` substituted, durations trimmed from the `Passed!` lines).
  - Panel 4.5 label: "captured 2026-09-16 · edited: paths, wrapping, one publish line, MSBuild prefixes" — the panel drops
    the `release/` publish line and the MSBuild diagnostic prefix of the image-index line. Panel 8.2 label: the SDK path,
    durations, the ` (net10.0)` suffix and the column padding of the `Passed!` lines are edited.
...
is more than about a third empty: the largest gaps are page 20 (29%), page 8 (27%) and page 11 (22%), measured with
...
|---|---|---|
| `L12.RatingVb` | VB library (`Rating.Quote`, `PremiumBreakdown`, `CoverageClass`) — illustrative rates | `rating` (not printed) |
...
| `L12.QuoteLambda.Tests` | xUnit + Amazon.Lambda.TestUtilities: rating totals, functions, generated wrapper, C# ↔ VB parity | `wrapper-test` |
...
| `L12.QuoteContainer.Tests` | xUnit + `WebApplicationFactory` + in-memory OTel exporter | `span-test` |
...
- Every host calls `L12.RatingVb`; rates stay illustrative and are labelled so.
...
- ⚠ Captured figures (zip sizes, image layers, publish output, triage output, test summary) are pasted constants; the
  size figures depend on package versions and the base image on 2026-09-16. Rerun the commands in 8.1 and update
